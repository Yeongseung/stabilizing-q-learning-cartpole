...
# 2. Failed buffer: Store only the last state before failure (1 per failure)
...
temp_ratio = 0.7  # 7:3 ratio (temporary:failed)
...
for episode in range(3000): # 5000 episodes
    ...
    while not game_over:
        ...
        total_buffer_size = len(temporary_buffer) + len(failed_buffer)
        if total_buffer_size >= batch_size and step_count % update_frequency == 0:
            batch = []
            
            # Sample with temporary:failed ratio
            temp_samples = int(batch_size * temp_ratio)
        ...
        if terminated or truncated:
            ...
            # Dual Buffer: Store in failed buffer only on failure
            # (temporary buffer already stores at each step)
            if terminated:  # Only terminated is a failure
            ...
            if failure_category is not None:
                print(f"episode: {episode}, Total reward: {total_reward}, Temp: {len(temporary_buffer)}, Failed: {len(failed_buffer)}, Reason: {failure_category}")
            else:
                print(f"episode: {episode}, Total reward: {total_reward}, Temp: {len(temporary_buffer)}, Failed: {len(failed_buffer)}")
            break
